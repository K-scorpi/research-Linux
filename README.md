...
Для запуска:
1) docker build .
2) docker run -itd --privileged "id образа" systemd
3) docker exec -it "id контейнера" bash
Для появления в логах информации о sshrc и web shell отдельно(__web shell затирает остальные логи!__)
